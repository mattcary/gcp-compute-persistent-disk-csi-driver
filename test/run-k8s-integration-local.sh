...
${PKGDIR}/bin/k8s-integration-test --run-in-prow=false \
--service-account-file=${GCE_PD_SA_DIR}/cloud-sa.json --do-driver-build=false \
--deploy-overlay-name=stable-master --storageclass-files=sc-standard.yaml,sc-balanced.yaml,sc-ssd.yaml \
--test-focus="External.Storage" --gce-zone="us-central1-b" \
--deployment-strategy=gce \
--local-k8s-dir=/clank/go/src/k8s.io/kubernetes \
--num-nodes=3
